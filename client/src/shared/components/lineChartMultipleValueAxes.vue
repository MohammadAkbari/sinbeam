...
const initChart = () => {
  ...
  props.labels.forEach((item, index) => {
    ...
    const yRendererMain = index == props.indexTwoAxios ? yRenderer1 : yRenderer;
    const yAxisMain = index == props.indexTwoAxios ? yAxis1: yAxis
    ...
    if (chart.yAxes.indexOf(yAxisMain) > 0) {
      yAxisMain.set("syncWithAxis", chart.yAxes.getIndex(0));
    }
    ...
    yRendererMain.grid.template.set("strokeOpacity", 0.05);

    yRendererMain.labels.template.setAll({
      fontSize: 10,
    // fontFamily:'IRANSans'
    });


    yRendererMain.labels.template.set("fill", series.get("fill"));
    yRendererMain.setAll({
